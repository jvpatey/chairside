...
export function getWorkerPhotoStoragePath(workerId: string, extension = 'jpg'): string {
  return `${workerId}/profile.${extension}`;
}
...
export async function uploadWorkerPhotoFromBase64(
  workerId: string,
  base64: string,
  contentType: string,
  ...
  const supabase = getSupabaseClient();
  const storagePath = getWorkerPhotoStoragePath(
    workerId,
...
export async function deleteWorkerPhoto(
  workerId: string,
  storagePath?: string | null,
): Promise<void> {
  const supabase = getSupabaseClient();
  const path = storagePath ?? getWorkerPhotoStoragePath(workerId);
...
export async function getWorkerPhotoSignedUrl(storagePath: string): Promise<string | null> {
  const supabase = getSupabaseClient();
...
export async function getWorkerPhotoDownloadRequest(storagePath: string): Promise<{
  url: string;
