...
import { updateClinicMembershipProfile } from './clinicOrganization';

const PHOTO_BUCKET = 'clinic-member-photos';
const MAX_PHOTO_BYTES = 2 * 1024 * 1024;
const ALLOWED_CONTENT_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp']);

export function getClinicMemberPhotoStoragePath(
  organizationId: string,
  membershipId: string,
  extension = 'jpg',
): string {
  return `${organizationId}/members/${membershipId}/profile.${extension}`;
}
...
export async function uploadClinicMemberPhotoFromBase64(
  organizationId: string,
  membershipId: string,
  base64: string,
  contentType: string,
  ...
  const supabase = getSupabaseClient();
  const storagePath = getClinicMemberPhotoStoragePath(
    organizationId,
    membershipId,
  ...
  await updateClinicMembershipProfile(membershipId, {
...
export async function deleteClinicMemberPhoto(
  organizationId: string,
  membershipId: string,
  ...
  const supabase = getSupabaseClient();
  const path =
    storagePath ?? getClinicMemberPhotoStoragePath(organizationId, membershipId);
...
export async function getClinicMemberPhotoSignedUrl(
  storagePath: string,
): Promise<string | null> {
  const supabase = getSupabaseClient();
...
export async function getClinicMemberPhotoDownloadRequest(storagePath: string): Promise<{
  url: string;
