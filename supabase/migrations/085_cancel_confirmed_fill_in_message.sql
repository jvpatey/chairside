-- Store cancellation notes and require a clinic message when cancelling confirmed fill-ins.

alter table public.applications
  add column if not exists status_note text,
  add column if not exists status_closed_by text;
...
alter table public.applications
  ...
  check (
    ...
    or status_closed_by in ('clinic', 'worker')
  );
