-- Optional list of doctors at the practice, shown on the public clinic profile.
alter table public.clinic_profiles
  add column if not exists practice_doctors jsonb not null default '[]'::jsonb;

comment on column public.clinic_profiles.practice_doctors is
  'Array of {id, name, title?, bio?, photo_storage_path?} objects listing doctors at the practice.';

-- Scrub doctor names when a clinic account is deactivated.
