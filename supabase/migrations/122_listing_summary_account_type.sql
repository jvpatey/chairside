-- Expose account_type on marketplace listing summaries so clients can
-- distinguish dental groups from single clinics on browse cards.
-- Return type change requires drop + recreate.

drop function if exists public.list_clinic_listing_summaries(text);
drop function if exists public.get_clinic_listing_summary(uuid);
...
as $$
  select
    ...
    coalesce(cp.account_type, 'individual') as account_type
