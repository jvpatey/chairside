-- Allow group managers to browse/start fill-in outreach using the org clinic profile.
-- Previously these RPCs used auth.uid() as clinic_id, which fails for managers
-- (their user id is not the organization / clinic_profiles.id).
...
begin
  ...
  if not exists (
    select 1 from public.profiles
    where id = v_actor_id and role = 'clinic'
  ) then
    raise exception 'Only clinics can start fill-in outreach';
  end if;

  v_clinic_id := coalesce(
    public.get_clinic_organization_id_for_user(v_actor_id),
    v_actor_id
  );
  ...
  if p_shift_date is not null and v_created_conversation then
    ...
    values (v_conversation_id, v_actor_id, v_shift_details_message, false, true);
  end if;

  insert into public.messages (conversation_id, sender_id, body, trigger_sms_alert, suppress_notification)
  values (v_conversation_id, v_actor_id, v_message_body, v_send_sms, false);
