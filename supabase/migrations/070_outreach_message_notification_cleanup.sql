-- Suppress Pingram notifications for internal/system-style messages (e.g. auto shift details).

alter table public.messages
  add column if not exists suppress_notification boolean not null default false;

comment on column public.messages.suppress_notification is
  'When true, the notify edge function skips Pingram dispatch for this message insert.';
